...
require('@prisma/sdk');
require('prettier');
require('../../dist/utils-22d9d904.cjs.dev.js');
require('prompts');
...
require('../../dist/types-for-lists-4a4a6e65.cjs.dev.js');
...
async function setupTestEnv(_ref) {
  ...
  const artifacts = await artifacts_dist_keystoneNextKeystoneArtifacts.getCommittedArtifacts(graphQLSchema, config);
  ...
  if (!_alreadyGeneratedProjects.has(hash)) {
    ...
    await artifacts_dist_keystoneNextKeystoneArtifacts.writeCommittedArtifacts(artifacts, artifactPath);
    await artifacts_dist_keystoneNextKeystoneArtifacts.generateNodeModulesArtifacts(graphQLSchema, config, artifactPath);
  }
  ...
  const {
    ...
    disconnect,
    createContext
  } = getKeystone(artifacts_dist_keystoneNextKeystoneArtifacts.requirePrismaClient(artifactPath));
  const {
    expressServer: app,
    apolloServer

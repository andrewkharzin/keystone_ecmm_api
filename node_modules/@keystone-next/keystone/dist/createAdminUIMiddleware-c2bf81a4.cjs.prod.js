...
var session_dist_keystoneNextKeystoneSession = require('../session/dist/keystone-next-keystone-session.cjs.prod.js');
var initConfig = require('./initConfig-ea0b3b38.cjs.prod.js');
var createApolloServer = require('./createApolloServer-49225e32.cjs.prod.js');
var url = require('url');
...
const createExpressServer = async (config, graphQLSchema, createContext) => {
  ...
  if ((_config$server3 = config.server) !== null && _config$server3 !== void 0 && _config$server3.extendExpressApp) {
    ...
    const createRequestContext = async (req, res) => createContext({
      sessionContext: config.session ? await session_dist_keystoneNextKeystoneSession.createSessionContext(config.session, req, res, createContext) : undefined,
      req
    });

    (_config$server4 = config.server) === null || _config$server4 === void 0 ? void 0 : _config$server4.extendExpressApp(expressServer, createRequestContext);
  }

  if (config.files) {
    var _config$files$local$s, _config$files$local;

    expressServer.use('/files', express__default["default"].static((_config$files$local$s = (_config$files$local = config.files.local) === null || _config$files$local === void 0 ? void 0 : _config$files$local.storagePath) !== null && _config$files$local$s !== void 0 ? _config$files$local$s : initConfig.DEFAULT_FILES_STORAGE_PATH));
  }

  if (config.images) {
    var _config$images$local$, _config$images$local;

    expressServer.use('/images', express__default["default"].static((_config$images$local$ = (_config$images$local = config.images.local) === null || _config$images$local === void 0 ? void 0 : _config$images$local.storagePath) !== null && _config$images$local$ !== void 0 ? _config$images$local$ : initConfig.DEFAULT_IMAGES_STORAGE_PATH));
  }
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const context = createContext({
        sessionContext: session ? await session_dist_keystoneNextKeystoneSession.createSessionContext(session, req, res, createContext) : undefined,
        req
      });

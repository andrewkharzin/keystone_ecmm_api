...
var core = require('./core-7942250b.cjs.prod.js');
var sqlite = require('./sqlite-6dd56161.cjs.prod.js');
var _objectWithoutProperties = require('@babel/runtime/helpers/objectWithoutProperties');
var graphqlTsSchema = require('./graphql-ts-schema-4c18b24a.cjs.prod.js');
var graphql$1 = require('graphql');
var graphqlErrors = require('./graphql-errors-a907a8e6.cjs.prod.js');
...
var typesForLists = require('./types-for-lists-7c049b0a.cjs.prod.js');
...
var fields_types_image_utils_dist_keystoneNextKeystoneFieldsTypesImageUtils = require('../fields/types/image/utils/dist/keystone-next-keystone-fields-types-image-utils.cjs.prod.js');
var crypto = require('crypto');
var stream = require('stream');
var filenamify = require('filenamify');
var slugify = require('@sindresorhus/slugify');
var fields_types_file_utils_dist_keystoneNextKeystoneFieldsTypesFileUtils = require('../fields/types/file/utils/dist/keystone-next-keystone-fields-types-file-utils.cjs.prod.js');
var fetch = require('node-fetch');
var FormData = require('form-data');
var cuid = require('cuid');
var packagePath = require('./package-path-261b12e7.cjs.prod.js');
  ...
  for (const [key, list] of Object.entries(initialisedLists)) {
    ...
    for (const fieldKey of Object.keys(list.fields)) {
      ...
      if ((fieldFilterFields === null || fieldFilterFields === void 0 ? void 0 : (_fieldFilterFields$co = fieldFilterFields.contains) === null || _fieldFilterFields$co === void 0 ? void 0 : _fieldFilterFields$co.type) === graphql$1.GraphQLString) {
        ...
        possibleSearchFields.set(fieldKey, (fieldFilterFields === null || fieldFilterFields === void 0 ? void 0 : (_fieldFilterFields$mo = fieldFilterFields.mode) === null || _fieldFilterFields$mo === void 0 ? void 0 : _fieldFilterFields$mo.type) === sqlite.QueryMode.graphQLType ? 'insensitive' : 'default');
      }
    }
...
const appTemplate = (adminMetaRootVal, graphQLSchema, _ref, apiPath, isLiveReload) => {
  ...
  const allViews = adminMetaRootVal.views.map(views => {
    ...
    views = views.replace(/\?[A-Za-z0-9]+$/, ''); // during a live reload, we'll have paths from a webpack compilation which will make the paths
...
const generateAdminUI = async (config, graphQLSchema, adminMeta, projectAdminPath, isLiveReload) => {
  ...
  const userPagesDir = path__namespace["default"].join(process.cwd(), 'admin', 'pages');
  const userPagesEntries = await fastGlob__default["default"]('**/*.{js,jsx,ts,tsx}', {
    cwd: userPagesDir
  });

  for (const filename of userPagesEntries) {
    const outputFilename = path__namespace["default"].join('pages', filename);
    const path = path__namespace["default"].relative(path__namespace["default"].dirname(path__namespace["default"].join(projectAdminPath, outputFilename)), path__namespace["default"].join(userPagesDir, filename));
...
function getAdminMetaSchema(_ref) {
  ...
  const isAccessAllowed = config.session === undefined ? undefined : (_config$ui$isAccessAl = (_config$ui = config.ui) === null || _config$ui === void 0 ? void 0 : _config$ui.isAccessAllowed) !== null && _config$ui$isAccessAl !== void 0 ? _config$ui$isAccessAl : _ref2 => {
    let {
      session
    } = _ref2;
...
async function validateDelete(_ref2) {
  let {
    list,
    hookArgs
  } = _ref2;
...
async function createSingle(_ref, list, context, operationAccess, writeLimit) {
  let {
    data: rawData
  } = _ref;
...
async function updateMany(_ref2, list, context, provider) {
  let {
    data
  } = _ref2;
  const writeLimit = pLimit__default["default"](provider === 'sqlite' ? 1 : Infinity); // Check operation permission to pass into single operation
...
function flattenMultiDbFields(fields, data) {
  return Object.fromEntries(Object.entries(data).flatMap(_ref5 => {
    let [fieldKey, value] = _ref5;
    const {
      dbField
    } = fields[fieldKey];

    if (dbField.kind === 'multi') {
      return Object.entries(value).map(_ref6 => {
        let [innerFieldKey, fieldValue] = _ref6;
...
function getGraphQLSchema(lists, provider, extraFields) {
...
function createImagesContext(config, cloudAssets) {
  ...
  return {
    ...
    getDataFromRef: async ref => {
      const imageRef = fields_types_image_utils_dist_keystoneNextKeystoneFieldsTypesImageUtils.parseImageRef(ref);
...
function createFilesContext(config, cloudAssets) {
  ...
  return {
    ...
    getDataFromRef: async ref => {
      const fileRef = fields_types_file_utils_dist_keystoneNextKeystoneFieldsTypesFileUtils.parseFileRef(ref);
    ...
    getDataFromStream: async (stream$1, originalFilename) => {
      ...
      if (mode === 'cloud') {
        const {
          filesize
        } = await cloudAssets().files.upload(stream$1, filename);
        return {
          mode,
          filesize,
          filename
...
async function getCloudAssetsAPI(_ref2) {
  ...
  if (!cloudAssetsConfigCache.has(apiKey)) {
    const res = await fetch__default["default"]('https://init.keystonejs.cloud/api/rest/config', {
      headers
    });
  ...
  return {
    images: {
      ...
      async metadata(id, extension) {
        const res = await fetch__default["default"](`${imageMetaUrl}/${id}.${extension}`);
      ...
      async upload(buffer, id) {
        const res = await fetch__default["default"](imageUploadUrl, {
          method: 'POST',
          body: formUploadBody({
            data: buffer,
            fieldName: 'image',
    ...
    files: {
      ...
      async metadata(filename) {
        const res = await fetch__default["default"](`${fileMetaUrl}/${filename}`);
      ...
      async upload(stream, filename) {
        const res = await fetch__default["default"](fileUploadUrl, {
          method: 'POST',
          body: formUploadBody({
            data: stream,
            fieldName: 'file',
...
exports.DEFAULT_FILES_STORAGE_PATH = DEFAULT_FILES_STORAGE_PATH;
exports.DEFAULT_IMAGES_STORAGE_PATH = DEFAULT_IMAGES_STORAGE_PATH;

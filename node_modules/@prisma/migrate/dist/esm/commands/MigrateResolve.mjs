...
${chalk.bold.green(getCommandWithExecutor("prisma migrate resolve --rolled-back 20201231000000_example"))}`);
    ...
    if (args["--applied"]) {
      if (typeof args["--applied"] !== "string" || args["--applied"].length === 0) {
        throw new Error(`--applied value must be a string like ${chalk.bold.green(getCommandWithExecutor("prisma migrate resolve --applied 20201231000000_example"))}`);
      }
      ...
      if (typeof args["--rolled-back"] !== "string" || args["--rolled-back"].length === 0) {
        throw new Error(`--rolled-back value must be a string like ${chalk.bold.green(getCommandWithExecutor("prisma migrate resolve --rolled-back 20201231000000_example"))}`);
      }
      await ensureCanConnectToDatabase(schemaPath);
      const migrate = new Migrate(schemaPath);
      try {
